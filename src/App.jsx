...
// Construir query para gatos (cat)
...
export default function App() {
  ...
  const runQuery = async () => {
    ...
    try {
      if (mode === 'students') {
        ...
        setResult(response.data.cat ? [response.data.cat] : [])
      }
    } catch (err) {
      setError(err)
    } finally {
  ...
  return (
    <div style={{ padding: 20, fontFamily: 'Arial, sans-serif' }}>
          ...
          {(mode === 'students' ? STUDENT_FIELDS : CAT_FIELDS).map(f => (
            <label key={f} style={{ marginRight: 12 }}>
              <input
                ...
                checked={
                  mode === 'students'
                    ? selectedStudentFields.includes(f)
                    : selectedCatFields.includes(f)
                }
      ...
      <div style={{ marginTop: 16 }}>
        <button onClick={runQuery} style={{ padding: '8px 14px' }}>
          Run Query
        </button>
      </div>
